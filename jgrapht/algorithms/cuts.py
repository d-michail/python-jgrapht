...
from .._internals._results import _wrap_cut, _wrap_gomory_hu_tree, _build_vertex_set
...
def mincut_stoer_wagner(graph):
    ...
    return _wrap_cut(graph, cut_source_partition_handle, cut_weight)
...
def gomory_hu_gusfield(graph):
    ...
    return _wrap_gomory_hu_tree(graph, handle)
...
def oddmincutset_padberg_rao(graph, odd_vertices, use_tree_compression=False):
    ...
    """
    odd_set = _build_vertex_set(graph, odd_vertices)
    (
        cut_weight,
        cut_source_partition_handle,
