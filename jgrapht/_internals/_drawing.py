...
class _JGraphTLongLayoutModel2D(_HandleWrapper, LayoutModel2D):
    ...
    def get_vertex_location(self, vertex):
        res = backend.jgrapht_lx_drawing_layout_model_2d_get_vertex(self.handle, vertex)
        return _point2d_class(*res)

    def set_vertex_location(self, vertex, point_2d):
        backend.jgrapht_lx_drawing_layout_model_2d_put_vertex(
            self.handle, vertex, *point_2d
        )

    def is_fixed(self, vertex):
        return backend.jgrapht_lx_drawing_layout_model_2d_get_fixed(self.handle, vertex)

    def set_fixed(self, vertex, fixed):
        backend.jgrapht_lx_drawing_layout_model_2d_set_fixed(self.handle, vertex, fixed)
