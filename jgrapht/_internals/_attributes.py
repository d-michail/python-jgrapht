...
class _JGraphTAttributesRegistry(_HandleWrapper):
    ...
    def put(self, name, category, attr_type=None, default_value=None):
        ...
        encoded_attr_type = bytearray(attr_type, encoding='utf-8') if attr_type is not None else None
        encoded_default_value = bytearray(default_value, encoding='utf-8') if default_value is not None else None
        backend.jgrapht_attributes_registry_register_attribute(
            self._handle, encoded_name, encoded_category, encoded_attr_type, encoded_default_value
        )
